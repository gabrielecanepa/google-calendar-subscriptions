...
Define a subscription following its [definition](#schemasubscription):

```js
const newSubscription = {
  // Optional name that will be used by the related calendar.
...
Create a calendar linked to the subscription using [`subscriptions.insert`](#calendarsubscriptionsinsertparams-paramsresourcesubscriptionsinsert-options-methodoptions--promiseschemasubscription):
  
```js
// Create a calendar and sync the specified subscription.
const subscription = await client.subscriptions.insert(newSubscription)

// The returned subscription will contain the ID of the newly created calendar.
...
// Retrieve the calendar events and check if the function was applied.
const { data: { items } } = await calendar.events.list({ calendarId: subscription.calendarId })
console.log(items.every(item => item.summary === item.summary.toUpperCase())) // => true
```

You can now start syncing the subscription using [`subscriptions.sync`](#calendarsubscriptionssyncparams-paramsresourcesubscriptionssync-options-methodoptions--promisevoid):

```js
await client.subscriptions.sync(subscription)
```

Subscription functions can also be asynchronous:
...
const newAsyncSubscription = {
  ...
  fn: async events => {
    ...
    return events.map(event => {
      // use the fetched data to modify events...
    })
  },
}

const asyncSubscription = await client.subscriptions.create(newAsyncSubscription)
```

And multiple subscriptions can be synced at once:

```js
await client.subscriptions.sync([subscription, asyncSubscription])
```
...
Creates a calendar and syncs the specified subscription.
...
Namespace of the Google Calendar client with additional subscription definitions (omitted in the API documentation).
